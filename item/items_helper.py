...
def is_filler(item: ProcessedMinecraftItem):
    return item.classification == ItemClassification.filler and item.fill_type != 3
...
def add_optional_items_to_pool(world: FabricMinecraftWorld, value: str, item: str, count: int, total_items: int):
    if value in world.options.randomized_abilities.value:
        return add_items_to_pool(world, item, count, total_items)
    else:
        return total_items