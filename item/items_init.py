...
def create_items(world: FabricMinecraftWorld):
    ...
    total_items = add_optional_item(world, "Spawn_Point", "Sleeping", total_items)
    total_items = add_optional_item(world, "Wither_Summoning", "Wither Summoning", total_items)
    total_items = add_optional_item(world, "Trading", "Villager Trading", total_items)
    total_items = add_optional_item(world, "Bartering", "Piglin Bartering", total_items)

    total_items = add_optional_item(world, "Brewing", "Brewing", total_items)
    total_items = add_optional_item(world, "Enchanting", "Enchanting", total_items)
    total_items = add_optional_item(world, "Smithing", "Smithing", total_items)
    total_items = add_optional_item(world, "Misc_Stations", "Other Crafting Stations", total_items)

    total_items = add_optional_item(world, "Bucket", "Bucket Recipes", total_items)
    total_items = add_optional_item(world, "Igniter", "Flint and Steel Recipes", total_items)
    total_items = add_optional_item(world, "Minecarts", "Minecart Recipes", total_items)
    total_items = add_optional_item(world, "Brush", "Brush Recipes", total_items)
    total_items = add_optional_item(world, "Spyglass", "Spyglass Recipes", total_items)
    total_items = add_optional_item(world, "Shears", "Shear Recipes", total_items)
    total_items = add_optional_item(world, "Ender_Eye", "Eye of Ender Recipes", total_items)
    total_items = add_optional_item(world, "Fishing", "Fishing Rod Recipes", total_items)
    total_items = add_optional_item(world, "Bottles", "Glass Bottle Recipes", total_items)
    total_items = add_optional_item(world, "Compacting", "Resource Compacting Recipes", total_items)
    total_items = add_optional_item(world, "Shield", "Shield Recipes", total_items)
    total_items = add_optional_item(world, "Bundles", "Bundle Recipes", total_items)
    total_items = add_optional_item(world, "TNT", "TNT Recipes", total_items)

    # Progressive Progression Items
    total_items = add_optional_items_to_pool(world, "Tools", "Progressive Tools", 4, total_items)
    total_items = add_optional_items_to_pool(world, "Weapons", "Progressive Weapons", 4, total_items)
    total_items = add_optional_items_to_pool(world, "Smelting", "Progressive Smelting", 2, total_items)
    total_items = add_optional_items_to_pool(world, "Armor", "Progressive Armor", 5, total_items)
    total_items = add_optional_items_to_pool(world, "Archery", "Progressive Archery", 2, total_items)
    ...
    # Drehmal Items
    if world.options.randomized_mythicals:
        for item in drehmal_mythicals:
            total_items = add_item_to_pool(world, item.name, total_items)
    if world.options.randomized_legendaries:
        for item in drehmal_legendaries:
            total_items = add_item_to_pool(world, item.name, total_items)
    if world.options.randomized_terminus_towers != 0:
        for item in drehmal_terminus_towers:
            total_items = add_item_to_pool(world, item.name, total_items)
    if world.options.randomized_quest_items:
        for item in drehmal_quest_items:
            total_items = add_item_to_pool(world, item.name, total_items)
            if item.name.startswith("Stone of"):
                for i in range(1, 8):
                    total_items = add_item_to_pool(world, item.name, total_items)
    if world.options.randomized_relics:
        for item in drehmal_relics:
            total_items = add_item_to_pool(world, item.name, total_items)
...
def create_local_fill_items(world: FabricMinecraftWorld):
    ...
    while filler_size > 0:
        if len(location_map) == 0:
            raise OptionError(
                "Another AP world is attempting to mess with Drehmal's prefill locations, please go politely inform them of this blunder posthaste!")
